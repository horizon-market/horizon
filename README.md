...
- **Undeployed contract change pending**: maker orders now carry a per-market budget, enforced on chain when an order is admitted to the router. Publication is two transactions (`Aqua.ship`, then `HorizonSwapVM.admitCurve`) and an unadmitted order never fills. It needs a new router, executor and Subgraph; nothing has been broadcast. Rule: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#per-market-order-budgets). Deployment: [OPERATIONS.md](OPERATIONS.md#per-market-order-budgets-required-redeployment).
