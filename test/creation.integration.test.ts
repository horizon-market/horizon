...
const world: WorldConfig = { appId: 'app_test', action: 'create-market', environment: 'staging', access: 'granted', verifyUrl: 'https://developer.worldcoin.org' };
...
const payload = (payer = '0.0.99', nonce = randomUUID()) => Buffer.from(JSON.stringify({ x402Version: 2, scheme: 'exact', network: 'hedera:testnet', payload: { payer, nonce } })).toString('base64');
...
test('a creation request is drafted, approved, paid once and created once', { timeout: 30_000 }, async () => {
  ...
  const requirements = target.requirements(issued.payment, RESOURCE);
  assert.equal(requirements.maxAmountRequired, '100000000');
  assert.equal(requirements.payTo, '0.0.4242');

  const settled = await target.submitPayment(id, token, payload(), RESOURCE);
...
test('payment retries are idempotent and never settle twice', { timeout: 30_000 }, async () => {
  ...
  const replay = await target.submitPayment(id, token, payload(), RESOURCE);
  assert.equal(replay.replay, true);
  const other = await target.submitPayment(id, token, payload('0.0.1000'), RESOURCE);
...
test('an ambiguous settlement parks the request for reconciliation instead of charging again', { timeout: 30_000 }, async () => {
  ...
  const { id, token, request } = await newRequest(target);
  await target.approve(id, token, draftHash(draftSchema.parse(request.draft)));
  await target.requirePayment(id, token, RESOURCE);
  await assert.rejects(() => target.submitPayment(id, token, payload(), RESOURCE),
...
test('the human discount applies once per credential per UTC day, after server verification', { timeout: 30_000 }, async () => {
  ...
  const verified = await target.verify(first.id, first.token, {
    nullifierHash: nullifier, merkleRoot: `0x${'12'.repeat(32)}`, proof: `0x${'34'.repeat(64)}`, verificationLevel: 'orb',
  });
  ...
  await target.verify(second.id, second.token, {
    nullifierHash: nullifier, merkleRoot: `0x${'12'.repeat(32)}`, proof: `0x${'34'.repeat(64)}`, verificationLevel: 'orb',
  });
  ...
  await assert.rejects(() => target.verify(second.id, second.token, {
    nullifierHash: nullifier, merkleRoot: `0x${'12'.repeat(32)}`, proof: `0x${'34'.repeat(64)}`, verificationLevel: 'orb',
...
test('an unavailable verifier reports the state and leaves the standard price in force', { timeout: 30_000 }, async () => {
  ...
  await target.approve(id, token, draftHash(draftSchema.parse(request.draft)));
  await assert.rejects(() => target.verify(id, token, {
    nullifierHash: `0x${'ab'.repeat(16)}`, merkleRoot: `0x${'12'.repeat(32)}`, proof: `0x${'34'.repeat(64)}`, verificationLevel: 'orb',
...
test('another holder of a request id cannot read or advance it', { timeout: 30_000 }, async () => {
  ...
  for (const wrong of [undefined, '', 'not-the-token', token.slice(0, -1) + (token.endsWith('a') ? 'b' : 'a')]) {
    ...
    await assert.rejects(() => target.submitPayment(id, wrong, payload(), RESOURCE), (error: unknown) => error instanceof WorkflowError && error.httpStatus === 401);
  }
...
test('the HTTP creation resource answers 402 once, then records one payment for both client kinds', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    for (const requesterKind of ['browser', 'agent'] as const) {
      ...
      const requirements = await required.json() as { x402Version: number; accepts: { maxAmountRequired: string; payTo: string; scheme: string; network: string }[] };
      ...
      assert.equal(requirements.accepts[0]!.maxAmountRequired, '100000000');

      const paid = await fetch(`${base}/api/creation/requests/${body.request.id}/payment`, { method: 'POST', headers: { ...auth, 'x-payment': payload(`0.0.${requesterKind === 'browser' ? 11 : 22}`) }, body: '{}' });
      assert.equal(paid.status, 200);
      assert.ok((paid.headers.get('x-payment-response') ?? '').length > 0);
      const again = await fetch(`${base}/api/creation/requests/${body.request.id}/payment`, { method: 'POST', headers: { ...auth, 'x-payment': payload('0.0.999') }, body: '{}' });
