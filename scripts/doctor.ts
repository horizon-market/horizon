...
if (env.EVM_RPC_URL) {
  await check('Sepolia chain and contracts', async () => {
    ...
    return 'Sepolia ID, code presence at configured addresses, and 6 USDC decimals checked. Deployed bytecode identity still needs verification.';
  });
  if (env.EVM_DEPLOYER_ADDRESS) await check('Deployer funding', async () => {
    ...
    return 'Positive ETH and test USDC balances; deployment gas sufficiency is not yet estimated.';
  });
...
if (env.GRAPH_QUERY_URL) await check('Graph query endpoint', async () => {
  ...
  return `Live endpoint at block ${reply.data._meta.block.number}; Horizon event indexing still needs a deployment.`;
});
...
report('Custom Horizon router', 'pending', 'Local official-source probe only; prediction-specific validation and deployment belong to Phase 1.');
